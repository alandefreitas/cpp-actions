...
The \`one-per-vs-year\` policy selects one MSVC toolset version per Visual Studio release year (e.g., one for VS 2019, one for VS 2022, one for VS 2026), plus the latest available MSVC version. This is the default policy for MSVC. Only applicable to MSVC; for other compilers it falls back to \`latest\`.
...
When the flag is unspecified, the action will infer the flag from the compiler name and its version.`
    ...
    ldflags: {
        ...
        default: [] as string[],
        crossTransform: parseSuggestionsCross,
        description: `A multi-line list of linker flags to be used with each range of compiler version. Each line has the format:

\`<compiler-name>[ <compiler-range|compiler-factor>]: <ldflags>\`

For instance, \`gcc >=13.1: -static\` indicates that the linker flag \`-static\` should be used to test \`gcc\` with any version in the semver range \`>=13.1\`.
...
Omitting \`<compiler-range|compiler-factor>\` is equivalent to it being set to \`*\` and will append the flags to all versions of the compiler.`
    ...
    appendLdflags: {
        ...
        default: [] as string[],
        crossTransform: parseSuggestionsCross,
        description: `A multi-line list of linker flags to append for each range of compiler version. Unlike \`ldflags\`, which replaces the entire flag string, this option appends to the flags already generated by the action (e.g. sanitizer flags, coverage flags).

Each line has the format:

\`<compiler-name>[ <compiler-range|compiler-factor>]: <ldflags>\`

For instance, \`gcc >=13: -static\` appends \`-static\` to the linker flags for any \`gcc\` entry with version \`>=13\`.
...
- \`ldflags\`: The recommended linker flags to be used by this entry. It reflects the values of special factors, such as sanitizers and coverage, which require matching flags at link time.
